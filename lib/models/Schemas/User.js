...
var logger = require('../../framework/logger').init('User');
...
module.exports.addUser = function(userName, email, password, callback) {
	...
	user.save(function(err, savedUser) {
		myCache.set( savedUser.id, user, function( err, success ){
			if( !err && success ){
...
module.exports.getUsersListById = function(listOfClientIds, callback) {
	myCache.mget( listOfClientIds, function( err, docs ){
		if( !err ){
			if(Object.keys(docs).length < listOfClientIds.length) {
				// key not found
				logger.audit('getUsersListById', "getUsersListById doesn't find in cache. found.length = " 
						+ Object.keys(docs).length + ", listOfClientIds.length = " + listOfClientIds.length);
				...
				User.find({'_id': { $in: listOfClientIds} }, function(err, result) {
					if(err != null) {
						...
						for(var i = 0; i < result.length; i++){
							myCache.set(result[i]._id, result[i]);
...
module.exports.getUserById = function( clientId , callback ) {
	myCache.get( clientId , function( err, client ){
		if( !err ){
			if(client == undefined){
				// key not found
