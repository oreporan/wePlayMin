...
// 		it('create a user, a leagues, a game that is set for 3 seconds from now, with frequency == -1, this test will have to wait 3 seconds ', function(done) {
...
//         var leagues = test.generateLeague(leagueName, client);
//         test.saveLeagueToDB(leagues, function(leagues){
// 					test.sendRequest(Constants.POST, path.BASE_GAME + path.PATH_GAME_ADDGAME, {'client-id' : client._id}, {leagueId:leagues.id, matchDay : matchDay, users: client.id }, function (err, res) {
// 						expect(err).to.be.null;
...
//
//   this.timeout(methodTimeOutInSec * 1000);
//   it('create a user, a leagues, a game that is set for 3 seconds from now, with frequency == 0, so the game should reoccur next week ', function(done) {
//     var userName = 'createMatch';
//     var leagueName = 'createMatchLeague';
//     var matchDay = wpDate.dateAdd(Date(), 'second', matchDayInSec);
//     var user = test.generateUser(userName);
//     test.saveUserToDB(user, function(client) {
//       var leagues = test.generateLeague(leagueName, client, 0);
//       test.saveLeagueToDB(leagues, function(leagues){
//         test.sendRequest(Constants.POST, path.BASE_GAME + path.PATH_GAME_ADDGAME, {'client-id' : client._id}, {leagueId:leagues.id, matchDay : matchDay, users: client.id }, function (err, res) {
//           expect(err).to.be.null;
//           expect(res).to.have.status(200);
...
//           // Wait for cronjob to be ready
//           setTimeout(function(){
//             // We should have two games in the leagues
//             test.getLeagueById(leagues._id, function(err, result) {
