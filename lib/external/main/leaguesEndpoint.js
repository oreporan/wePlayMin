...
function addLeague(req, res) {
	...
	leagues.addLeague(name, body.admin, body.frequency, body.games, body.users, function(league, err) {
		if(err != null) {
...
function getLeagueById(req, res) {
	...
	leagues.getLeagueById(leagueId, function(league, err) {
		if(err != null) {
...
router.get(path.PATH_LEAGUES_ADDUSERTOLEAGUE, addUserToLeague);
function addUserToLeague(req, res) {
	var body = req.body;
	var clientId = body.clientId;
	var leagueId = body.leagueId;
	logger.audit('addUserToLeague' , 'adding clientId: ' + clientId + ' to LeagueId');
	leagues.addUserToLeague(clientId, leagueId, function(league, err) {
		if(err != null) {
...
router.post(path.PATH_LEAGUES_REMOVEUSERFROMLEAGUE, removeUserFromLeague);
function removeUserFromLeague(req, res) {
	var body = req.body;
	var clientId = body.clientId;
	var leagueId = body.leagueId;
	logger.audit('removeUserFromLeague' , 'adding clientId: ' + clientId + ' to LeagueId');
	leagues.removeUserFromLeague(clientId, leagueId, function(league, err) {
		if(err != null) {
			logger.warn('removeUserFromLeague' , 'could not remove user with clientId: ' + clientId + ' to League: ' + leagueId
					+ ' to DB, reason : ' + err);
			res.send(wpResponse.sendResponse(false, null, "Could not remove user to league", err));
			return;
		}
		logger.audit('removeUserFromLeague' , 'clientId: ' + clientId + ' was removeed to league: ' + leagueId + ' to DB, admin : ' + league.admin);
		res.send(wpResponse.sendResponse(true, league));
		return;
	});
};
...
router.get(path.PATH_LEAGUES_GETALLUSERLEAGUES + ':clientId', getAllUserLeagues);
function getAllUserLeagues(req, res) {
	var clientId = req.params.clientId;
	logger.audit('getAllUserLeagues' , 'get clientId: ' + clientId);
	leagues.getAllUserLeagues(clientId, function(leagues, err) {
		if(err != null) {
			logger.warn('getAllUserLeagues' , 'could not get all user with clientId: ' + clientId + ' leagues ' + ', reason : ' + err);
			res.send(wpResponse.sendResponse(false, null, "Could not get league by ID", err));
			return;
		}
		logger.audit('getAllUserLeagues' , 'found all user with clientId: ' + clientId + ' leagues');
