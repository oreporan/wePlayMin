...
router.get(path.PATH_GAME_CLOSEGAME + '/:gameId', closeGame);
function closeGame(req, res) {
	var gameId = req.params.gameId;
  var clientId = req.get(Constants.CLIENT_ID_HEADER_KEY);
	logger.audit('closeGame' , 'creating a game in league - '+ leagueId +', to be played at:' + matchDay);
	games.closeGame(gameId, clientId, function(err, game) {
		if(err != null) {
			logger.warn('closeGame' , 'could not create game, reason : ' + err);
			res.send(wpResponse.sendResponse(false, null, "Could not add game", err));
			return;
		}
		logger.audit('closeGame' , 'game: ' + game.id + ' was saved');
		res.send(wpResponse.sendResponse(true, game));
		return;
	});
};
