...
passport.use(new GoogleStrategy({
  ...
  function(accessToken, refreshToken, profile, done) {
    logger.audit('googleAuthentication' , 'user ' + email + ' authenticating via google');
...
router.post(path.PATH_AUTHENTICATE_FACEBOOK,
  ...
  function (req, res) {
    var user = req.user;
    logger.audit('googleAuthentication' , 'user ' + email + ' authenticated via facebook');
    users.findOrCreate(user.username, user.email, user.password, user.authId, function(err, result){
      if(err) {
...
router.post(path.PATH_AUTHENTICATE_GOOGLE,
  ...
  function (req, res) {
    logger.audit('googleAuthentication' , 'user ' + email + ' authenticated via google');
    var user = req.user;
    users.findOrCreate(user.username, user.email, user.password, user.authId, function(err, result){
      if(err) {
        res.send(wpresponse.sendResponse(false, null, err, err.errmsg));
...
// app.get(path.PATH_AUTHENTICATE_GOOGLE + '/callback',
...
function invalidParams(email, password, name) {
  if(email == null || email.length < 1) return true;
  if(password == null || password.length < 1) return true;
...
function authenticateViaSocialNetwork(accessToken, profile, done, method) {
  ...
  var user = {email : email, password : password, username : userName, authId : {'id' : id, 'method' : method } };
