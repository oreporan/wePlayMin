    ...
    test.saveUserToDB(user, function(client) {
      ...
      test.saveLeagueToDB(league, function(league){
				...
				test.saveGamesToDBBulk(gamesArray, function(listOfGames) {
					...
					// Make request
					chai.request(app)
					.post(path.ROOT + path.BASE_GAME + path.PATH_GAME_GETGAMELIST)
					.set(Constants.CLIENT_ID_HEADER_KEY, client._id) // Sets the clientID header
					.send({games:listOfGames})
					.end(function (err, res) {
						expect(err).to.be.null;
						expect(res).to.have.status(200);
						var body = res.body;
  ...
  it('Add a new user to an existing game ', function(done) {
    ...
    test.saveUsersToDBBulk(listOfUsers, function(clients) {
      ...
      test.saveLeagueToDB(league, function(league){
        ...
        .send({leagueId:league.id, matchDay : Date(), users: clients[0].id })
        .end(function (err, res) {
          // Game created, now we add a new user to this game
          var responseText = res.body.responseText;
          var gameId = responseText.games[0];
          chai.request(app)
          .post(path.ROOT + path.BASE_GAME + path.PATH_GAME_ADDUSERTOGAME_WITH_GAMEID + '/' + gameId)
          .set(Constants.CLIENT_ID_HEADER_KEY, clients[1]._id) // Sets the clientID header
          .send({leagueId:league.id, matchDay : Date(), users: clients[1].id })
          .end(function (err, res) {
            expect(err).to.be.null;
            var body = res.body;
            expect(body).to.have.property('success').with.true;
