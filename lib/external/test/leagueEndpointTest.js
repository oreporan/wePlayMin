...
describe('leagueEndpointTest.js, This test classes tests the leagueEndpoint class', function() {
					...
					test.sendRequest(method, requestPath, headers, {position : "LM"}, function (err, res) {
						...
						test.getUserById(clients[1]._id, function(err, result) {
							...
							test.getLeagueById(leagueFromDb.id, function(err, result) {
								...
								expect(result.invites.length).eql(0);
								done();
							})
						});
					});
	...
	describe('#addInviteUserToLeague()', function() {
		it('Create a league, then push an invite user to that league', function(done) {
			...
			test.saveUsersToDBBulk(usersList, function(clients){
				...
				test.saveLeagueToDB(league, function(leagueFromDb) {
					...
					// send request
					test.sendRequest(method, requestPath, headers, {isInvite : true, position : "CF"}, function (err, res) {
						expect(err).to.be.null;
						var body = res.body;
						expect(body).to.have.property('success').with.true;
						expect(body).to.have.property('responseText');
	...
	describe('#removeInviteUserToLeague()', function() {
		it('Create a league, then push an invite user to that league, then remove him', function(done) {
			...
			test.saveUserToDB(user, function(client) {
				...
				test.saveLeaguesToDBBulk(leagueList, function(leaguesFromDB) {
					...
					test.sendRequest(method, requestPath, headers, body, function (err, res) {
						...
						expect(body).to.have.property('responseText');
						var responseText = body.responseText;
						expect(Object.keys(responseText).length).eql(3);
						done();
					});
				});
			});
	...
	describe('#addAdmin()', function() {
		it('Create a leaugue with an admin, then add another user to the league and make him admin', function(done) {
			...
			test.saveUsersToDBBulk(usersList, function(clients) {
				...
				league.users.push(test.generateLeagueUser(clients[1], false));
