...
module.exports.doRequest = function(req,res, next) {
	logger.audit('doRequest', '================== START OF REQUEST ==================');
	logger.audit('doRequest' , 'new request coming in: ' + req.method + ' ' + req.ip + ' ' + req.path);
	if(req.path.indexOf('/auth/') == 0) {
		logger.audit('doRequest', 'going to authenticate, no need to validate');
		next();
	} else {
		validateRequest(req, function( err ) {
			if(err == null) {
				logger.audit('doRequest' ,  'Request has passed validation, routing to: ' + req.path);
				next();
			} else {
				logger.error('doRequest' ,  'Request failed validation - ' + err);
	...
	users.getUserById(clientId, function(err, result) {
		if( err != null || result == null ) {
			...
			if(req.path.indexOf('/games/') == 0) {
				...
				if(!leagueId) {
					callback('No leagueId: ' + leagueId + ', can not route to: ' + req.path);
				} else {
