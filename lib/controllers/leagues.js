...
module.exports.addUserToLeague = function(clientId, leagueId, isInvite, position, callback) {
	users.updateLeagues(clientId, leagueId, true, function(err, result) {
		...
		var userObject = generateLeagueUser(result, isInvite, position);
...
var updateLeague = module.exports.updateLeague = function(leagueId, params, callback) {
	...
	// Helper methods
	var generateLeagueUser = module.exports.generateLeagueUser = function(userObj, isInvite, position) {
		var realPosition = isValidPosition(position) ? position : Constants.LEAGUE_POSITION_CM.name;
		return {'_id' : userObj._id, 'position' : realPosition, form : [], isInvite : isInvite,  date: Date()}
	  return result;
	}
