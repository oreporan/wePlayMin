    ...
    if(err){
      ...
      async.parallel({
        ...
        game : function(callback) {
          if(clientId) {
            updateGameAttendingStatus(leagueObj._id, game.id, clientId, 1, callback);
          }
        },
...
function updateGameStatus(gameObj) {
  var numOfPlayers = sumAttendingPlayers(gameObj.users);
...
// Helper methods
function generateGameUsers(userArray, attending) {
  var result = [];
  var attending = attending ? attending : Constants.ATTENDING_UNDECIDED;
  for(var i = 0 ; i < userArray.length ; i++) {
    result.push({'_id' : userArray[i], 'attending' : attending});
  }
  return result;
}

function sumAttendingPlayers(listOfUsers) {
  var count = 0;
