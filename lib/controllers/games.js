...
/* Closes the game and starts the game! */
      ...
      if( err != null ) {
        ...
        leagues.getLeagueById(leagueId, function(err, result){
          if(err != null || result == null) {
            ...
            matches.startMatch(matchId, callback);
          }
        });
      }
    });
...
module.exports.addUserToGame = function(gameId, clientId, callback) {
  // First we see the situation with the players in the game
    ...
    if(err == null) {
      ...
      leagues.getLeagueById(gameResult.leagueId, function(err, leagueResult){
        if(err == null) {
          ...
          buildGame(maxNumOfPlayersPerTeam, gameResult, callback);
        }
      });
    }
  });
