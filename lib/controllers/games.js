		...
		if(err){
			...
			async.parallel({
				...
				if(err != null) {
					...
					// Notify all league users that a game has been added
...
var addUserToGamesBulk = module.exports.addUserToGamesBulk = function(gamesArray, leagueUser, callback) {
	logger.audit('addUserToGamesBulk', 'adding user : ' + leagueUser._id + ' to all ' + gamesArray.length + ' games in the league');
